...
export default{
    createRecipe:async({ user_id, title, thumbnail, description, ingredients, instructions})=>{
        ...
        for (const instruction of instructions) {
            ...
            VALUES (?, ?, ?, ?,?)`
                , [recipe_id, instruction.title, instruction.img_url, instruction.description,instruction.step_order]);
        }
    ...
    getRecipe: async (recipeId) =>{
        const res = await conn.query(
            `SELECT title, img_url, description, step_order FROM Instructions WHERE recipeId = ?`,[recipeId]
        );
        const ret = await conn.query(
            `SELECT name, quantity FROM Ingredient WHERE recipeId = ?`, [recipeId]
        );
        const reu = await conn.query(
            `SELECT r.title as title, r.description as description, r.created_at as created_at, u.user_name as user_name FROM recipes as r LEFT JOIN users as u on u.user_id = r.user_id WHERE r.recipe_id = ?`, [recipeId]
        );
