...
authController.post("/duplicate", async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json(apiResponse.success({ message: "사용 가능한 유저 이름입니다." }));
  } catch (e) {
    next(e);
  }
});
...
authController.post("/login", async (req, res, next) => {
  ...
  try {
    ...
    const findUser = await userRepository.findByUsernameAndPassword(
      username,
      password,
    );
    ...
    await userRepository.updateSessionIdByUserId(
      findUser.userId,
      req.sessionID,
    );
...
authController.post("/logout", async (req, res, next) => {
  try {
    ...
    res.status(200).json(apiResponse.success({ message: "로그아웃 성공" }));
  } catch (e) {
    next(e);
  }
});
